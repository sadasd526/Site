...
def buy_products():
    profit = get_all_tradeable() #get_profit()

    for i in profit:
        for v in get_products(i['id'])['items']:
            print(v)
            if (float(i['price']) - float(i['purchasePrice'])) < 0 and get_info({"items":[v['goodId']],"currency":"rub"})[0]['canBuy'] > 0:
